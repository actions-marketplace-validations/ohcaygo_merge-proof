...
function harness(search='',fetchImpl){
 ...
 const context=vm.createContext({document:{getElementById:get,createElement:element},location,URLSearchParams,setInterval(){},fetch:async(url,init)=>{calls.push({url,init});return fetchImpl?fetchImpl(url,init):{ok:true,json:async()=>({installations:[{id:2,account:'fixture'}]})};}});
