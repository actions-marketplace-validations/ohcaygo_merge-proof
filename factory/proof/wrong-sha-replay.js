...
local.findings = local.findings.filter(
  (f) => !["CI_RAN_ON_FINAL_HEAD", "HUMAN_APPROVAL_PRESENT"].includes(f.id),
);
...
fs.writeFileSync(
  path.join(__dirname, "public-results/wrong-sha-withholding.json"),
  JSON.stringify(
    {
      method:
        "CONTROLLED WITHHOLDING REPLAY, not a claim about the live repository: remove all landed-SHA checks from the authentic click #3781 capture; retain only genuine candidate-SHA checks, select one of those check names, and require evidence for the original landed SHA. No SHA/outcome/approval is fabricated. Original live capture remains unchanged.",
